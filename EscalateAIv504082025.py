...
# EscalateAI Full Version with Email & Excel Parsing, NLP, Kanban, SLA Alerts & Excel Export
...
def sidebar_content():
    ...
    # Escalation filter: All or only Escalated (priority High)
    escalation_filter = st.sidebar.radio(
        "Escalation Filter",
        options=["All", "Escalated Only"],
        index=0,
...
def display_kanban(df, status_filter, escalation_filter):
...
def main():
    ...
    df = load_escalations_df()
    display_kanban(df, status_filter, escalation_filter)
